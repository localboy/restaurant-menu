...
class MyLunchAPITEST(MyLunchTestCase):
    def setUp(self):
        ...
        self.employee = Employee.objects.create(
            user=self.user, designation='Manager', phone='435345434')
        ...
        self.the_day_before_yesterday = self.current_date - \
            timezone.timedelta(days=2)

    def test_create_restaurant(self):
        response = self.client.post(get_api_url('restaurants'), {
    ...
    def test_upload_menu(self):
        ...
        file = SimpleUploadedFile(
            menu_file, data.read(), content_type='multipart/form-data')
        payload = {"file": file}
        response = self.client.post(
            get_api_url('restaurants/upload-todays-menu'),
            payload,
            format="multipart"
        )
    ...
    def test_create_employee(self):
        ...
        response = self.client.post(
            get_api_url('employees'), data, format='json')
        self.assertCreated(response)
    ...
    def test_menu_votting(self):
        ...
        votting = self.client.post(get_api_url(
            'restaurants/{}/vote'.format(menu_id)))
        ...
        again_votting = self.client.post(
            get_api_url('restaurants/{}/vote'.format(menu_id)))
